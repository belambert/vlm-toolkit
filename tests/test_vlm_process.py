...
def test_process_batch_returns_results(mock_model, mock_processor, tmp_images):
    from vlm_toolkit.vlm_process import process_batch
...
@patch("vlm_toolkit.vlm_process.load_model")
@patch("vlm_toolkit.vlm_process.get_device", return_value="cpu")
@patch("vlm_toolkit.vlm_process._run_inference", return_value=[])
@patch("vlm_toolkit.vlm_process.prepare_vlm_batch", return_value=(MagicMock(), []))
...
@patch("vlm_toolkit.vlm_process.get_device", return_value="cpu")
@patch(
    "vlm_toolkit.vlm_process._run_inference",
    return_value=[{"file_name": "x.png", "output": "y"}],
)
@patch(
    "vlm_toolkit.vlm_process.prepare_vlm_batch", return_value=(MagicMock(), ["x.png"])
)
def test_vlm_process_batching(
    mock_prep, mock_infer, mock_device, mock_load, tmp_images
):
    from vlm_toolkit.vlm_process import vlm_process
...
@patch(
    "vlm_toolkit.vlm_process._run_inference",
    return_value=[{"file_name": "img.png", "output": "caption"}],
)
@patch(
    "vlm_toolkit.vlm_process.prepare_vlm_batch", return_value=(MagicMock(), ["img.png"])
)
def test_vlm_process_writes_jsonl(
    mock_prep, mock_infer, mock_device, mock_load, tmp_images
):
    from vlm_toolkit.vlm_process import vlm_process
